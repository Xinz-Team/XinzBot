...
module.exports = async(xinz, anj, welcome, left) => {
    ...
    if (anj.action === 'add'){
        if (anj.participants[0] === xinz.user.jid){
            ...
            xinz.sendMessage(anj.jid, `Hello ${botName}, Darling use #menu`, MessageType.text)
        } else if (isWelcome){
            try {
            ...
            xinz.sendMessage(anj.jid, await getBuffer(pic), MessageType.image, {caption: `hello Darling @${anj.participants[0].split("@")[0]}, Welcome Darling(❤️ω❤️) ${groupName}`, contextInfo: {"mentionedJid": [anj.participants[0]]}})
        }
